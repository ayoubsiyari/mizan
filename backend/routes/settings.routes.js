...
router.get('/users', requireRole('admin'), asyncHandler(async (req, res) => {
  const rows = await all(
    `SELECT id, username, email, first_name, last_name, phone, role, is_active, is_verified, last_login_at, created_at
     FROM users WHERE firm_id = ? AND deleted_at IS NULL ORDER BY created_at DESC`,
    [req.user.firm_id]
  );
...
router.post('/users', requireRole('admin'), asyncHandler(async (req, res) => {
  const { email, password, firstName, lastName, role = 'lawyer', phone } = req.body || {};
  ...
  await run(
    `INSERT INTO users (id, firm_id, username, email, password_hash, first_name, last_name, phone, role, is_verified)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
    [id, req.user.firm_id, username, email, hash, firstName, lastName, phone || null, role]
  );
  const row = await get(
...
router.put('/users/:id', requireRole('admin'), asyncHandler(async (req, res) => {
  const allowed = ['first_name', 'last_name', 'phone', 'role', 'is_active'];
  const sets = [];
  const params = [];
  for (const k of allowed) {
    const bodyKey = k === 'first_name' ? 'firstName' : k === 'last_name' ? 'lastName' : k === 'is_active' ? 'isActive' : k;
    if (req.body[bodyKey] !== undefined) { sets.push(`${k} = ?`); params.push(req.body[bodyKey]); }
  }
